...
class OptimizeJh:
    ...
    def _generate_all_patterns(self):
        ...
        if self.has_roll_orders:
            ...
            roll_only_patterns = [dict(p) for p in self.patterns]
    ...
    def _build_mixed_patterns(self):
        ...
        if not self.sheet_items or not self.roll_items:
            return mixed_patterns

        roll_items_sorted = sorted(self.roll_items, key=lambda i: self.item_info[i], reverse=True)

        def add_candidate(pat):
            if not self._is_valid_mixed_pattern(pat):
                return
            key = frozenset(pat.items())
            if key in seen:
                return
            seen.add(key)
            mixed_patterns.append(dict(pat))
        ...
        for sheet_item in self.sheet_items:
            ...
            for ri in roll_items_sorted:
                ...
                tw = sw + rw
                cnt = 1
                while tw <= self.max_width and (1 + cnt) <= self.max_pieces:
                    if tw >= self.min_width:
                        add_candidate({sheet_item: 1, ri: cnt})
                    cnt += 1
                    tw += rw
            ...
            if self.max_pieces >= 3:
                for i, ri1 in enumerate(roll_items_sorted):
                    ...
                    for ri2 in roll_items_sorted[i:]:
                        ...
                        tw = sw + rw1 + rw2
                        if tw > self.max_width or tw < self.min_width:
                            continue
                        if ri1 == ri2:
                            add_candidate({sheet_item: 1, ri1: 2})
                        else:
                            add_candidate({sheet_item: 1, ri1: 1, ri2: 1})
    ...
    def _estimate_roll_underproduction(self, patterns):
        ...
        order_items = list(self.roll_demands.keys())

        x = [solver.NumVar(0, inf, f'xr_{j}') for j in range(num_patterns)]
        under = [solver.NumVar(0, inf, f'ur_{d}') for d in range(len(order_items))]
        over = [solver.NumVar(0, inf, f'or_{d}') for d in range(len(order_items))]

        for d_idx, demand_key in enumerate(order_items):
            supply_terms = []
            for j in range(num_patterns):
                contrib = self._get_pattern_contribution(patterns[j], demand_key)
                if contrib > 0:
                    supply_terms.append(x[j] * contrib)
            supply = solver.Sum(supply_terms) if supply_terms else 0
            solver.Add(supply + under[d_idx] - over[d_idx] == self.roll_demands[demand_key])

        solver.Minimize(solver.Sum(under))
        status = solver.Solve()
