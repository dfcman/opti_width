...
[핵심 제약조건]
- 최소/최대 슬리터 폭 (min_sl_width, max_sl_width)
...
# --- 복합폭 및 패턴 관련 페널티 ---
COMPOSITE_USAGE_PENALTY = 3000.0  # 복합폭 사용 페널티 (단폭이 가능하면 단폭 선호, 필요시 복합폭 허용)
...
# --- 복합폭 규격 혼합 설정 ---
ALLOW_MIXED_WIDTH_COMPOSITE = 'N'  # 'Y': 다른 규격을 섞어서 복합폭 생성 가능, 'N': 동일 규격끼리만 복합폭 생성

# --- 2단계 최적화(Two-Stage) 파라미터 ---
TWO_STAGE_TOLERANCE = 0.05  # Stage 1 비용 대비 허용 오차 (10%)


class RollOptimizeSt:
    """
    롤지 단폭(ST) 최적화 클래스
    
    단폭 단위로 절단 패턴을 구성하여 주문을 충족시키는 최적화를 수행합니다.
    ...
    def __init__(
        ...
        Args:
            ...
            min_sl_width: 복합폭 최소 폭 (mm)
            max_sl_width: 복합폭 최대 폭 (mm)
        ...
        self.min_sl_width = self.min_sl_width
        self.max_sl_width = self.max_sl_width
    ...
    def _prepare_items(self):
        ...
        [처리 순서]
        1. 단폭(1폭) 아이템 등록 - min_sl_width~max_sl_width 범위 내
        ...
        # 1폭은 sl_trim 미적용 (트림은 2폭 이상 복합폭에서만 적용)
        for item, width in self.base_item_widths.items():
            if width <= 0:
                continue
            
            # 1폭은 원본 지폭 그대로 사용 (sl_trim 미적용)
            if self.min_sl_width <= width <= self.max_sl_width:
                self.item_info[item] = width  # 원본 폭 저장 (trim 없음)
        ...
        # 단폭 가능 아이템도 복합폭 생성 (페널티로 단폭 선호, 필요시 복합폭 허용)
        for base_item, base_width in all_base_for_composite.items():
            for num_repeats in range(self.composite_min, self.composite_max + 1):
                ...
                trim = self.sl_trim if num_repeats >= 2 else 0
                composite_w_with_trim = composite_width + trim
        ...
        # 혼합 복합폭 생성 (다른 규격 조합)
        if ALLOW_MIXED_WIDTH_COMPOSITE != 'Y':
            logging.info("  [혼합 복합폭] 동일 규격만 허용 (ALLOW_MIXED_WIDTH_COMPOSITE='N')")
            return
        ...
        def backtrack(start_idx, composition, total_width, total_pieces, current_roll_length):
            ...
            # 2폭 이상이면 sl_trim 적용, 1폭이면 미적용
            trim = self.sl_trim if total_pieces >= 2 else 0
            composite_w_with_trim = total_width + trim
            ...
            for idx in range(start_idx, len(base_candidates)):
                ...
                # 다음 아이템 추가 시 2폭 이상이 되므로 sl_trim 적용하여 범위 체크
                next_pieces = total_pieces + 1
                next_trim = self.sl_trim if next_pieces >= 2 else 0
                if (total_width + width + next_trim) > self.max_sl_width:
                    continue
    ...
    def _solve_master_problem(self, is_final_mip=False):
        ...
        if is_final_mip:
            try:
                ...
                logging.info("Trying Gurobi Direct Solver RollOptimizeST (gurobipy)...")
