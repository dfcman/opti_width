...
class RollOptimize:
    ...
    def _is_valid_pattern_width(self, total_width):
        """
        패턴의 총폭이 유효한지 검증합니다.
        paper_type이 6631인 경우 4500~4700 범위는 제외합니다.
        """
        # paper_type이 제한 목록에 있으면 4500~4700 범위 제외
        if str(self.paper_type) in DISALLOWED_PATTERN_WIDTH:
            if 4300 < total_width < 4700:
                return False
        return True
    ...
    def _add_pattern(self, pattern):
        ...
        if not self._is_valid_pattern_width(total_width):
            return False
