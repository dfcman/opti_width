...
PATTERN_COMPLEXITY_PENALTY = 0.001  # 패턴 복잡성에 대한 페널티
...
class SheetOptimize:
    def __init__(
        ...
        self.original_df_orders = df_spec_pre.copy() # 원본 주문 저장
    ...
    def _generate_initial_patterns(self):
        """
        초기 패턴 생성을 위해 두 가지 전략을 사용합니다.
        1. First-Fit-Decreasing 휴리스틱으로 다양한 품목이 섞인 패턴을 생성합니다.
        2. 각 복합폭 아이템을 최대한 채우는 '순수 품목 패턴'을 생성하여 다양성을 확보합니다.
        """
        print("\n--- 유효한 초기 패턴을 생성합니다 ---")
        ...
        # --- 1. FFD (First-Fit-Decreasing) 휴리스틱으로 혼합 패턴 생성 ---
    ...
    def _find_unproducible_width(self):
        """현재 패턴 목록으로 생산할 수 없는 수요 지폭을 찾습니다."""
        for width, demand in self.demands_in_rolls.items():
            ...
            for p in self.patterns:
                ...
                if can_produce:
                    break
            if not can_produce:
                print(f"[문제 원인] 지폭 {width} (수요: {demand}롤)을 생산할 수 있는 패턴이 현재 목록에 없습니다.")
                return width
        
        print("[원인 불명] 모든 수요 지폭에 대한 생산 패턴은 존재하지만, 다른 제약으로 인해 해를 찾지 못했습니다.")
        return None
    ...
    def run_optimize(self):
        ...
        while True:
            # 1. 현재 제외 목록을 기반으로 주문 및 관련 속성 재설정
            self.df_orders = self.original_df_orders[~self.original_df_orders['지폭'].isin(excluded_widths)]
            if self.df_orders.empty:
                final_results = {"error": "모든 주문이 제외되어 최적화를 진행할 수 없습니다."}
                break
            ...
            if len(self.order_widths) <= SMALL_PROBLEM_THRESHOLD:
                ...
                if not self.patterns:
                    final_results = {"error": "초기 유효 패턴을 생성할 수 없습니다. 제약조건이 너무 엄격할 수 있습니다."}
                    break
                ...
                for iteration in range(CG_MAX_ITERATIONS):
                    ...
                    if not master_solution or 'duals' not in master_solution:
                        print("\n--- 마스터 문제를 해결할 수 없습니다. 원인 분석 시작 ---")
                        unproducible_width = self._find_unproducible_width()
                        if unproducible_width:
                            print(f"--- 해당 지폭({unproducible_width})을 제외하고 최적화를 다시 시도합니다. ---")
                            excluded_widths.append(unproducible_width)
                            should_restart = True
                        else:
                            final_results = {"error": "마스터 문제를 해결할 수 없으며, 특정 문제 지폭을 찾지 못했습니다."}
            ...
            # 3. 최종 최적화
            if not self.patterns:
                final_results = {"error": "유효한 패턴을 생성할 수 없습니다."}
                break
            ...
            if not final_solution:
                ...
                if unproducible_width:
                    ...
                    final_results = {"error": "최종 해를 찾을 수 없습니다."}
                    break
            ...
            if excluded_widths:
                ...
                for w in excluded_widths:
                    print(f"  - 지폭: {w} mm (생산 가능한 패턴을 찾지 못함)")
                    excluded_summary.append({"지폭": w, "사유": "생산 가능 패턴 없음"})
