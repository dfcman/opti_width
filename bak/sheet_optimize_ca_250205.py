...
# 페널티 값
OVER_PROD_PENALTY = 10.0    # 과생산에 대한 페널티(1미터당 페널티 부여 값)
UNDER_PROD_PENALTY = 50.0  # 부족생산에 대한 페널티(1미터당 페널티 부여 값)
PATTERN_COUNT_PENALTY = 100000.0       # "칼 세팅(패턴 변경) 횟수를 줄여라!" (생산 효율을 위해 전체 패턴 종류를 줄임)
DISALLOWED_SINGLE_BASE_WIDTHS = {}  # 단일 사용을 금지할 주문 폭 집합
SINGLE_STRIP_PENALTY = 500000.0  # 패턴 내 단폭 아이템 개수에 비례하여 페널티 부여 (복합폭 x2 이상 사용 유도)
PATTERN_COMPLEXITY_PENALTY = 1.0  #  (복잡도 페널티) "한 패턴에 여러 규격을 섞지 마라!" (작업자가 헷갈리지 않게 단순한 구성을 선호하게 만듦)
MIXED_SHEET_LENGTH_PENALTY = 10.0  # [New] 패턴 내 다른 세로 길이 조합에 대한 페널티 (같은 세로끼리 조합 유도)
